...
workspace = r"D:\GIS_Database\AK_TCC\DEM\slope" #where all the quarter quads are stored
...
## Only process those rasters that intersect the study area
	if ((xmin >= xminA and xmin <= xmaxA) or (xmax >= xminA and xmax <= xmaxA)) and ((ymin >= yminA and ymin <= ymaxA) or (ymax >= yminA and ymax <= ymaxA)):
		...
		#copy over
		arcpy.CopyRaster_management(rast, outRast, pixel_type = "32_BIT_FLOAT", nodata_value = "-9999")
	else:
